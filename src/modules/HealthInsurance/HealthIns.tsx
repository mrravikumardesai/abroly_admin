...
const HealthIns = () => {
    ...
    const initDataApiCall = async () => {
        const { data, success } = await commonGetAPICalls("/health_in/list")
    ...
    const addApiCall = async () => {
        const { success } = await commonPostAPICall({ name: addCardDetails.name, details: addCardDetails.details, url: addCardDetails.url }, "health_in/add", true)
        if (success && success == true) {
            initDataApiCall()
            onOpenChangeAdd()
            setAddCardDetails({
